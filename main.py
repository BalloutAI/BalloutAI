...
class T5Finetuner(pl.LightningModule):
    ...
    def prepare_batch(self, questions: List[str], answers: List[str]) -> List[str]:
        ...
        labels = self.tokenizer.batch_encode_plus(
            list(answers), padding=True, truncation=True, return_tensors='pt')['input_ids']
...
class MyDataset(Dataset):
    def __init__(self, n_examples: int, min_digits: int, max_digits: int, operation: str,
        ...
        if balance:
            ...
            if train_dat== True:
                ...
                batch_1, labels_1 = self.load_cifar10_data('data_batch_1')
                batch_2, labels_2 = self.load_cifar10_data('data_batch_2')
                batch_3, labels_3 = self.load_cifar10_data('data_batch_3')
                batch_4, labels_4 = self.load_cifar10_data('data_batch_4')
                batch_5, labels_5 = self.load_cifar10_data('data_batch_5')
                X_trains = np.concatenate([batch_1,batch_2,batch_3,batch_4,batch_5], 0)
                Y_train = np.concatenate([labels_1,labels_2,labels_3,labels_4,labels_5], 0)
                X_train = self.return_photo(X_trains)
                
                gray_scale = np.dot(X_train[...,:3], [0.2989, 0.5870, 0.1140])

                f = np.reshape(gray_scale,(50000,1024))

                int_array = f.astype(int)
                str_array = int_array.astype(str)
                
                self.examples= str_array.tolist()
                # print((self.examples[0]))
                self.my_labels= Y_train.tolist()

            else:
                
                test, Y_test = self.load_cifar10_data('test_batch')
                # print(type(Y_test))
                X_test = self.return_photo(test)            
                gray_scale_te = np.dot(X_test[...,:3], [0.2989, 0.5870, 0.1140])
                ff = np.reshape(gray_scale_te,(10000,1024))
                int_array_te = ff.astype(int)
                int_array_tes = int_array_te.astype(str) 
                self.examples= int_array_tes.tolist()
                self.my_labels= Y_test
    ...
    def load_cifar10_data(self, filename):
        with open('C:/Users/moham/Desktop/Listops-scratch/cifar-dataset/'+ filename, 'rb') as file:
    ...
    def pre_process(self, data_string):

        data_string= data_string.replace('(', '')
    
        data_string= data_string.replace(')', '')

        test_list= data_string.split(" ")
    ...
    def __getitem__(self, idx):
        ...
        my_question= " " 
        # print("********")
        # print(len(self.examples[idx]))
        # print("********")
        for x in range(len(self.examples[idx])):
            # numbers["string{}".format(x)] = self.examples[idx][x]
            my_question+= " "
            my_question+= self.examples[idx][x]
            my_answer= self.my_labels[idx] 
        ...
        # print(c)
        # print(d)
        # print(len(my_question))
        # print(len(my_question[0]))
        # print("***************")
        # print(type(my_answer))
        # print("***************")
        return my_question, str(my_answer)

        # first_term, second_term,third_term ,fourth_term ,fifth_term, sixth_term , seventh_term , eigth_term,ninth_term  , tenth_term  ,eleven_term, twelve_term , thriteen_term , fourteen_term , fifteen_term , sixteen_term , seventeen_term , eighteen_term , ninteen_term , twenty_term   = self.examples[idx] # sadasda ,,fifth_term, sixth_term , seventh_term , eigth_term,ninth_term  , tenth_term  ,eleven_term, twelve_term , thriteen_term , fourteen_term , fifteen_term , sixteen_term , seventeen_term , eighteen_term , ninteen_term , twenty_term

        # if self.operation == 'addition':
        #     operation_term = 'plus'
        #     result = (first_term + second_term + third_term + fourth_term + fifth_term + sixth_term +seventh_term + eigth_term+ ninth_term  + tenth_term  + eleven_term+ twelve_term + thriteen_term + fourteen_term + fifteen_term + sixteen_term + seventeen_term + eighteen_term + ninteen_term + twenty_term )
        ...
        # third_term = self.convert_number(third_term, invert_number=self.invert_question)
        # fourth_term = self.convert_number(fourth_term, invert_number=self.invert_question)
        # fifth_term = self.convert_number(fifth_term, invert_number=self.invert_question)
        # sixth_term = self.convert_number(sixth_term, invert_number=self.invert_question)
        # seventh_term = self.convert_number(seventh_term, invert_number=self.invert_question)
        # eigth_term = self.convert_number(eigth_term, invert_number=self.invert_question)
        # ninth_term = self.convert_number(ninth_term, invert_number=self.invert_question)
        # tenth_term = self.convert_number(tenth_term, invert_number=self.invert_question)
        # eleven_term = self.convert_number(eleven_term, invert_number=self.invert_question)
        # twelve_term = self.convert_number(twelve_term, invert_number=self.invert_question)
        # thriteen_term = self.convert_number(thriteen_term, invert_number=self.invert_question)
        # fourteen_term = self.convert_number(fourteen_term, invert_number=self.invert_question)
        # fifteen_term = self.convert_number(fifteen_term, invert_number=self.invert_question)
        # sixteen_term = self.convert_number(sixteen_term, invert_number=self.invert_question)
        # seventeen_term = self.convert_number(seventeen_term, invert_number=self.invert_question)
        # eighteen_term = self.convert_number(eighteen_term, invert_number=self.invert_question)
        # ninteen_term = self.convert_number(ninteen_term, invert_number=self.invert_question)
        # twenty_term = self.convert_number(twenty_term, invert_number=self.invert_question)           
        
        # answer = self.convert_number(result, invert_number=self.invert_answer)

        # return f'What is {first_term} {operation_term} {second_term} {operation_term} {third_term} {operation_term} {fourth_term} {operation_term} {fifth_term} {operation_term} {sixth_term}  {operation_term} {seventh_term} {operation_term} {eigth_term} {operation_term} {ninth_term} {operation_term} {tenth_term} {operation_term} {eleven_term} {operation_term} {twelve_term} {operation_term} {thriteen_term} {operation_term} {fourteen_term} {operation_term} {fifteen_term} {operation_term} {sixteen_term} {operation_term} {seventeen_term} {operation_term} {eighteen_term} {operation_term} {ninteen_term} {operation_term} {twenty_term} ?', answer #  {operation_term} {fifth_term} {operation_term} {sixth_term}  {operation_term} {seventh_term} {operation_term} {eigth_term} {operation_term} {ninth_term} {operation_term} {tenth_term}{operation_term} {eleven_term} {operation_term} {twelve_term} {operation_term} {thriteen_term} {operation_term} {fourteen_term} {operation_term} {fifteen_term}{operation_term} {sixteen_term} {operation_term} {seventeen_term} {operation_term} {eighteen_term} {operation_term} {ninteen_term} {operation_term} {twenty_term}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_seq_length', type=int, default=2048, help='Maximum sequence length (in tokens).')
    ...
    dataset_train = MyDataset(n_examples=args.train_size, min_digits=args.min_digits_train,
                              ...
                              invert_answer=args.invert_answer, balance=args.balance_train, train_dat=True)

    dataset_val = MyDataset(n_examples=args.val_size, min_digits=args.min_digits_train,
                            max_digits=args.max_digits_train,
                            operation=args.operation, orthography=args.orthography,
                            base_number=args.base_number, invert_question=args.invert_question,
                            invert_answer=args.invert_answer, balance=args.balance_val, train_dat= False)
    ...
    dataset_test = MyDataset(n_examples=args.test_size,
                             ...
                             operation=args.operation, orthography=args.orthography,
                             base_number=args.base_number, invert_question=args.invert_question,
                             invert_answer=args.invert_answer, balance=args.balance_test, train_dat= False)
